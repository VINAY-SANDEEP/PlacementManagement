...
import { Search, Briefcase, Download, Calendar, ExternalLink, Filter, MapPin, ChevronLeft, ChevronRight, FileText } from "lucide-react";
...
export default function Placements({ darkMode, setDarkMode }) {
  ...
  // Helper check for recently posted notifications (3 days)
  ...
  const filteredPlacements = placements
    .filter((item) => {
      ...
      const matchBranch = branchFilter === "" || 
                          (Array.isArray(item.eligibleBranches) && item.eligibleBranches.some(b => b.toLowerCase().includes(branchFilter.toLowerCase())));
    ...
    .sort((a, b) => {
      ...
      // Sort by package value (parse simple float/int out of text if possible)
        ...
        <div className="glass p-6 rounded-2xl shadow-md grid md:grid-cols-12 gap-4 items-center">
          ...
          <div className="md:col-span-4 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white border-none focus:ring-2 focus:ring-primary-500 outline-none text-sm transition"
            />
          </div>
          ...
          <div className="md:col-span-3">
            <select
              ...
              className="w-full px-3 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white border-none focus:ring-2 focus:ring-primary-500 outline-none text-sm transition"
            >
        ...
        {loading ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {currentItems.map((item) => (
              <div
                key={item._id}
                className="glass rounded-2xl p-6 shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 relative overflow-hidden group flex flex-col justify-between border border-slate-100 dark:border-slate-850"
              >
                <div>
                  <div className="flex justify-between items-start mb-4">
                    <div className="h-14 w-14 rounded-xl bg-white dark:bg-slate-900 p-2 border border-slate-100 dark:border-slate-800 flex items-center justify-center shadow-sm">
                      <img
                    ...
                    <div className="flex flex-col items-end gap-1">
                      ...
                      {isRecentlyPosted(item.createdAt) && (
                        <span className="bg-rose-500 text-white text-[10px] px-2.5 py-0.5 rounded-full font-bold animate-pulse">
                          NEW
                        </span>
                      )}
                    </div>
                  ...
                  {/* Eligible branches tag stack */}
                  <div className="flex flex-wrap gap-1 mt-2">
                    {item.eligibleBranches?.map(branch => (
                      <span key={branch} className="text-[10px] px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-medium">
                        {branch}
                      </span>
                    ))}
                  </div>

                  <p className="text-slate-500 dark:text-slate-400 text-xs mt-3 line-clamp-3">
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 space-y-2 text-xs">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-slate-700 dark:text-slate-200">{item.ctc || "Best in Industry"}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">Eligibility details:</span>
                      <span className="font-semibold text-slate-700 dark:text-slate-200 text-right line-clamp-1">{item.eligibilityCriteria || "Refer Attached Doc"}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">Expected Exam:</span>
                      <span className="font-semibold text-slate-700 dark:text-slate-200">
                        {item.expectedExamDate ? new Date(item.expectedExamDate).toLocaleDateString() : "To Be Decided"}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-6 flex gap-3">
                  {item.pdf && (
                    <button
                      ...
                      className="flex-1 py-2 px-3 rounded-xl bg-slate-100 hover:bg-slate-250 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-semibold text-xs transition flex items-center justify-center gap-1"
                    >
                  ...
                  {item.formLink && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex-1 py-2 px-3 rounded-xl bg-primary-600 hover:bg-primary-500 text-white font-semibold text-xs transition flex items-center justify-center gap-1 shadow-md shadow-primary-500/10"
                    >
                      <span>Apply</span>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between border-t border-slate-200 dark:border-slate-800 pt-6">
            <div className="text-xs text-slate-500">
              Showing {indexOfFirstItem + 1} to {Math.min(indexOfLastItem, filteredPlacements.length)} of {filteredPlacements.length} placements
            </div>
              ...
              <div className="flex justify-between items-center px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900">
                ...
                <div className="flex items-center gap-3">
                  <a
                    href={previewPdfUrl}
                    download
                    className="p-2 rounded-full bg-slate-100 hover:bg-slate-250 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-350 text-xs font-semibold flex items-center gap-1 transition"
                  >
